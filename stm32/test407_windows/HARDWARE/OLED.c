 ...
 */
/* 位延时：168MHz 下 GPIO 翻转太快（一次电平变化只需 ~20ns），SCL 会跑到 1~2MHz，
...
void OLED_W_SCL(uint8_t BitValue)
{
	...
	GPIO_WriteBit(GPIO_OLED_PORT, GPIO_SCL_PIN, (BitAction)BitValue);
...
void OLED_W_SDA(uint8_t BitValue)
{
	...
	GPIO_WriteBit(GPIO_OLED_PORT, GPIO_SDA_PIN, (BitAction)BitValue);
...
void OLED_GPIO_Init(void)
{
	...
	/*将SCL和SDA引脚初始化为开漏模式*/
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE); // 正确

	GPIO_InitTypeDef GPIO_InitStructure;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_OD; // 开漏由 OType 指定
